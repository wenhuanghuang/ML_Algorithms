...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        ###############################################################################################
        # Use np.random.normal() with mean 0 and standard deviation 0.1 to initialize
        #   - self.params['W'] 
        #   - self.params['b']
        ###############################################################################################
        ...
        ###############################################################################################
        # Initialize the following two (gradients) with zeros
        #   - self.gradient['W']
        #   - self.gradient['b']
        ###############################################################################################
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # Implement the relu forward pass. Store the result in forward_output          #
        ################################################################################
        
        forward_output = np.maximum(0, X)
        self.mask = (forward_output > 0).astype(float)
    ...
    for module_name, module in model.items():
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ####################################################################################
                    # update the model parameter module.params[key] by a step of gradient descent.
                    ...
                    ###################################################################################################
                    # Update the model parameter module.params[key] by a step of gradient descent with momentum.
